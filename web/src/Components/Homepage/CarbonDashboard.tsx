...
const EMPTY_TRADING_SUMMARY: EmissionTradingSummary = {
  configuration: {
    venue_status: "not_configured",
    venue_name: null,
  ...
  return (
    <article className="donut-card analytics-chart-card" data-chart-id={chart.id}>
      ...
      {state === "available" ? (
        ...
        <div className="analytics-unavailable">{humanMetricValue(null, state)}</div>
      )}
...
const CarbonDashboard = () => {
  ...
  useEffect(() => {
    let active = true;
    get<Partial<EmissionTradingSummary>>(API_PATHS.EMISSION_TRADING_PUBLIC_SUMMARY())
      .then((response) => {
      ...
      .catch(() => {
        if (active) setTradingSummary(EMPTY_TRADING_SUMMARY);
      });
    return () => {
      active = false;
    };
  ...
  const isMarketConfigured = tradingSummary.configuration.venue_status !== "not_configured";
    ...
    <section className="carbon-dashboard" aria-busy={loading}>
      <div className="dashboard-container">
        ...
        <div className="analytics-filter-bar" aria-label="Registry analytics filters">
          ...
          <Select value={sector} onChange={setSector} options={[{ value: "", label: "All sectors" }, ...sectorOptions.map((value) => ({ value, label: value }))]} />
          <Select value={scheme} onChange={setScheme} options={[{ value: "", label: "All configured schemes" }, ...schemeOptions.map((value) => ({ value, label: value }))]} />
        ...
        <section className="section" aria-label="Emission ceiling and trading">
          ...
          <div className="donut-grid">
            ...
            <div className="donut-card"><div className="main-statistic"><div className="statistic-value">{tradingSummary.trading.companies}</div><div className="statistic-title">Market Participants in Trades</div></div></div>
            <div className="donut-card"><div className="main-statistic"><div className="statistic-value">{tradingSummary.trading.currency} {tradingSummary.trading.totalValueLAK.toLocaleString()}</div><div className="statistic-title">Entered Market Value</div></div></div>
          </div>
          <div className="registry-table-section"><EmissionCeilingTradingTabs /></div>
        </section>
      </div>
    </section>
  );
