...
import { capacityBuildingManagementApi as api } from "./api";
...
const CapacityBuildingManagement = () => {
  const { get, put, patch, delete: deleteRequest } = useConnection();
  const { userInfoState } = useUserContext();
  const [rows, setRows] = useState<CapacityRecord[]>([]);
  ...
  const [form] = Form.useForm<CapacityRecord>();
  const isAuthorized =
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      setRows((unwrap(response) as CapacityRecord[]) ?? []);
    } catch (error) {
      setRows([]);
  ...
  const edit = async (id: number) => {
    try {
      form.setFieldsValue(unwrap(await get(api.detail(id))) as CapacityRecord);
      setEditingId(id);
  ...
  const save = async (values: CapacityRecord) => {
    if (editingId === null) return;
    setSaving(true);
    try {
      const { id, archivedAt, ...payload } = values;
      await put(api.update(editingId), payload);
      message.success("Capacity building entry updated.");
  ...
  const archive = (id: number) => {
    Modal.confirm({
      title: "Archive this capacity building entry?",
      ...
      onOk: async () => {
        try {
          ...
          message.success("Capacity building entry archived.");
          await fetchRows();
        } catch (error) {
          message.error(errorText(error));
        }
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.success("Capacity building entry deleted.");
          await fetchRows();
        } catch (error) {
          message.error(errorText(error));
        }
  ...
  if (!isAuthorized) {
    return <Alert type="error" showIcon message="Only DNA/Ministry users can manage capacity building." action={<Link to="/">Back to home</Link>} />;
  }
  ...
  const columns = [
    ...
    { title: "Lifecycle", key: "lifecycle", render: (_: unknown, record: CapacityRecord) => record.archivedAt ? <Tag>Archived</Tag> : <Tag color="green">Public</Tag> },
    {
      title: "Actions",
      key: "actions",
      render: (_: unknown, record: CapacityRecord) => (
        <Space>
  ...
  return (
    <Card title="Capacity Building Management" extra={<Link to="/">Back to home</Link>}>
      ...
      <Modal title="Capacity building detail" visible={editorVisible} onCancel={() => setEditorVisible(false)} onOk={() => form.submit()} confirmLoading={saving} destroyOnClose width={760}>
...
export default CapacityBuildingManagement;
